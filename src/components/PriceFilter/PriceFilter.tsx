...
type PriceRangeT = { min: string, max: string }

const PriceFilter = () => {

    const [priceRange, setPriceRange] = useState<PriceRangeT>({ min: "0", max: "100" });

    const ValueConverter = (value: string) => {
        let NUM_Value = Number(value) || 0;
        if (NUM_Value > 100) {
            NUM_Value = 100
        }
    ...
    return (
        <div>
            <div className="px-3 py-4 flex items-center justify-between">
                <p className="text-lg text-darkBlack dark:text-lightSecondaryWhite pl-2">از</p>
                <input
                    type="text"
                    className="border-b border-secnodryMain caret-main bg-[#eee] dark:bg-darkThirdBlack outline-none LTR p-0.5 pl-3  text-2xl flex-1 w-full RTL"
                    value={priceRange.min}
                    onChange={e => setPriceRange(prv => ({ min: ValueConverter(e.target.value), max: prv.max }))}
                />
                <svg
                    ...
                    fill="none"
                    viewBox="0 0 15 16"
                    className="lg:w-6 lg:h-6 pt-1 mr-2"
                >
                    <path
            ...
            <div className="px-3 pb-4 flex items-center justify-between">
                <p className="text-lg text-darkBlack dark:text-lightSecondaryWhite pl-2">تا</p>
                <input
                    type="text"
                    className="border-b border-secnodryMain caret-main bg-[#eee] dark:bg-darkThirdBlack outline-none LTR p-0.5 pl-3  text-2xl flex-1 w-full"
                    value={priceRange.max}
                    onChange={e => setPriceRange(prv => ({ min: prv.min, max: ValueConverter(e.target.value) }))}
                />
                <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="20"
            ...
            <div className="px-3 h-14">
                <ReactSlider
                    ...
                    onChange={(value) => {
                        setPriceRange({ min: value[0].toString(), max: value[1].toString() })
                    }}
                    minDistance={1}
                    step={1}
            ...
            <button className="w-full bg-main transition-all duration-200 hover:bg-green-700/90 text-lightWhite py-2 rounded-b-lg">
                اعمال فیلتر
            </button>
        </div>
    )
